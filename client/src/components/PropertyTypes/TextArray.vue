  ...
  <div class="textarray-div">
	...
	class="textarray">
	  ...
	  :key="index">
        ...
        <span 
		v-if="value.includes('{{')" 
		v-html="parsedString(value)">
		</span>
        <!-- else there are no {{variables}} -->
        <span v-else>
			{{ value }}
		</span>
        <hr v-if="values.length > 1 && index != values.length-1" class="array-hr">
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    parseVariables(value) {
      ...
      for (const match of matches) {
		  ...
		  const variableObj = this.variables.find(variableObj => "{{" + variableObj.name + "}}" === match);
		  if (variableObj.value) {
			  // if there is an entered value for a variable, the variable tag is displayed different
			  openTag = 
			  `<span class="el-tag el-tag--info" style="color: #333;">
			   	<span class="el-tag el-tag--mini" style="margin: 0 10px 0 0;">
			  	<i class="el-icon-edit"></i>
			  </span>`;
            string = string.replace(regex, openTag + variableObj.value + closeTag);
		  } else {
			string = string.replace(/{{/g, openTag).replace(/}}/g, closeTag);
		  }
      }
